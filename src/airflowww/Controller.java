...
public class Controller {
	...
	// making dialogue boxes reference so user can change file name and maybe
	// where
	// it will be saved to:
	...
	public static void saveFile() throws FileNotFoundException {
		...
		for (int i = 0; i < xs.size(); i++) {
			output.println(xs.get(i) + "," + ys.get(i)); // output will be
															// "<x_coord>,<y_coord>"
		}
		output.close();
	}
	...
	public static void createFlowArrow() {
		// NOTE: have to have last vertex two times to properly close shape for
		// color fill
	...
	public static void createAirFoil(int numPoints, double upperCamber, double lowerCamber) {
		...
		for (int i = 0; i < numPoints; i++) {
			if (i < numPoints / 2) {
				double y = 5 * upperCamber * (0.2969 * Math.sqrt(x) - 0.1260 * x - 0.3516 * Math.pow(x, 2)
						+ 0.2843 * Math.pow(x, 3) - 0.1015 * Math.pow(x, 4));
				xs.add(x);
				ys.add(y);
				x += 1.0 / (numPoints / 2);
			} else {
				double y = -(5 * lowerCamber * (0.2969 * Math.sqrt(x) - 0.1260 * x - 0.3516 * Math.pow(x, 2)
						+ 0.2843 * Math.pow(x, 3) - 0.1015 * Math.pow(x, 4)));
				xs.add(x);
				ys.add(y);
				x -= 1.0 / (numPoints / 2);
