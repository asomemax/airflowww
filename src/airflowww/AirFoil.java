...
public class AirFoil extends Figure {
	...
	// leading edge would be the point in the front of airfoil with the maximum
	// curvature (smallest radius)
	...
	// chord will be connected from ptA to ptB, ptC will be the point that forms a
	// right angle with midpoint and either ptA or ptB
	private double findRadiusFromChordAndPt(Point2D.Double ptA, Point2D.Double ptB, Point2D.Double ptC) {
		...
		// r^2 = base^2 + (r - distance between ptC and midPt)^2 ==> r = (base^2 +
		// (distance between ptC and midPt)^2) / 2(distance between ptC and midPt)
	...
	// trailing edge would be the point in the front of airfoil with the maximum
	// curvature (smallest radius)
	...
	/**
	 * calculating drag force using F = 0.5 * C_d * p * V^2 * A reference:
	 * https://en.wikipedia.org/wiki/Drag_equation
	 * 
	 * @param massDensity
	 *            - mass density of fluid (air in this case), can be predefined
	 * @param flowSpeed
	 *            - velocity of air flow relative to object
	 * @param dragCoeff
	 *            - drag coefficient based on airfoil's shape, can use predefined
	 *            values
	 * @param refArea
	 *            - reference area, airfoils use the square of the chord length as
	 *            the reference area since airfoil chords are usually defined with a
	 *            length of 1,
	 * @return
	 */
	...
	public double findSurfaceArea(int start, int end) {
		double sum = 0;
